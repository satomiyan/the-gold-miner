...
public class Line {
    ...
    int endX = 600;
    int endY = 600;
    double length = 100;// Length of the line.
    double minLength = 100;// Minimum length the line can reach.
    double maxLength = 750;// Maximum length the line can reach.
    double angle = 0;
    int dir = 1;//Direction of the angle change.
    int state;// State of the line.
    GameWin frame;// Reference to the main game window.
    ...
    void logic() {
        ...
        for (GameObject obj : this.frame.objectList) {
            if (endX > obj.x && endX < obj.x + obj.width && endY > obj.y
                    && endY < obj.y + obj.height) {
                state = 3;// Set state to 3 if intersection occurs.
                obj.flag = true;// Set flag of the intersected object to true.
            }
        }
    }
